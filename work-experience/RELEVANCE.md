...
Each project and major deliverable in `work-experience/*.md` carries a **Relevance Score** (1–5) plus a one-sentence rationale. The score is the author's judgment of how heavily this deliverable should weigh on a resume — independent of how it's currently phrased on `will_cygan_resume.typ`.
...
| **4** | **Strong supporting** — drop first only when space-constrained | Quantified with clear ownership; narrower scope than a headline; a panel can grill it and walk away impressed. |
...
Resume reviewers (the 12 personas in `.claude/agents/`, invoked via `resume-panel`, `resume-panel-focus`, `resume-debate`, and `resume-interview-rehearsal`) treat the score as a **calibration prior, not gospel**:

- **Buried-lead flag.** RS 5/4 project missing from or weakly represented on `will_cygan_resume.typ` is a high-priority issue — cite the `work-experience/*.md` line.
...
**Should the resume itself (`will_cygan_resume.typ`) carry scores?** No. The resume is the distilled artifact; scores live on the raw log so agents can compare the two.
