...
- **Context:** In FY26 LinkedIn launched a company-wide initiative to migrate off Oracle. LBP used Oracle heavily as a shared monolithic database across 10 teams, meaning one team's incident degraded availability for every other team. Each team needed an independent MySQL database to scale its domain independently.
...
- Led design and delivery of **LBP Global Alerts** — LinkedIn commerce's payment-failure alerting system — recovering **$2M+ in funnel-attributed annual revenue**; Venice read path sized for **100K QPS** from LinkedIn Feed, with Flink stateful aggregation collapsing raw invoice events into ~500 net state updates/sec.
- Designed and led Oracle → MySQL migration of LinkedIn's Ordering Backend (**12 TB / 20B rows**) — rewrote **30,000+ lines across ~75 PRs** on JooQ + Flyway, ran a 6-week phased rollout validating **10,000+ orders at 1% tollgate** with zero data loss or downtime; maintained offline data-lake sync via **Brooklin CDC** alongside **GoldenGate** replication.
- Identified an opportunity during the Oracle deprecation to invest in shared migration infra before 10 LBP teams diverged on bespoke solutions; designed and shipped the **Oracle → MySQL Delegation Framework** — a Java library teams onboard by supplying their Oracle and MySQL implementations, integrating shadow reads, rollback hooks, and a Couchbase-backed entity-routing cutover layer — **adopted by 10 LBP domain teams** as the standard migration pattern.
- Cut purchase-flow read latency by eliminating N+1 query patterns across all ordering-workflow read paths — **p95 200 ms → 125 ms (37%)**, **avg 50 ms → 30 ms (40%)** on the 12 TB / 20B-row MySQL Ordering Backend, measured against a purpose-built Grafana migration dashboard.
- Built hourly Airflow+Trino **data-quality jobs** (row-count floors + rate-of-change assertions) for Ordering and Global Alerts offline datasets — converted two classes of silent pipeline failure into auto-detected alerts, including a **99% row-loss dataset migration** that error-based monitoring missed.
- Automated Venice TTL purging of >35-day records on the Global Alerts 100K-QPS cache — **40% cache-size reduction** and **50% cut in downstream Invoice Search API QPS**, removing a silent source of stale-hint amplification on every customer page-view.
- Built a rate-limited distributed deletion pipeline for the MySQL Ordering Backend — Airflow+Trino selects records and publishes them to Kafka; a middleman traffic distributor consumes the stream and drip-feeds gRPC delete calls to downstream services at a controlled rate — safely removed **300,000 corrupted order records** during an oncall incident and **unblocked deletion of billions of dormant legacy OMS rows**.
- Designed the **Context Repos pattern** for agentic code navigation (git submodules + curated context files) — piloted org-wide May 2025; **variant adopted by LinkedIn's Developer Productivity org Feb 2026** — making 30+ decomposed LBP microservices navigable to Claude Code.
- Designed Global Alerts for independent extensibility — **3 partner teams** (Ordering, Payments, Mobile) shipped Winback Promotions, Usage Billing, and Mobile Billing features on the platform under my guidance.
- Designed unified **optimistic locking** for the MySQL Ordering Backend — version-checked writes inside Temporal workflows converge activity-retry conflicts cleanly instead of silently clobbering, eliminating a class of conflicting-write failures on every update path.
