...
# dependencies = []
# ///
"""Evaluate the repository's Golden Resume across text and PDF representations."""
...
def forbidden_codepoints(text: str) -> list[str]:
...
def ordered_once(text: str, fields: list[str]) -> tuple[bool, dict[str, int], dict[str, int]]:
...
def association_results(text: str, windows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for window in windows:
        ...
        passes, counts, positions = ordered_once(segment, fields)
        results.append(
            {
                "label": window["label"],
...
def evaluate_text_view(text: str, oracle: dict[str, object]) -> dict[str, object]:
    ...
    ordered_positions = list(positions.values())
    associations = association_results(text, oracle["association_windows"])
    forbidden = forbidden_codepoints(text)
...
def xml_nodes(path: pathlib.Path) -> list[dict[str, object]]:
    root = element_tree.parse(path).getroot()
    nodes: list[dict[str, object]] = []
    for node in root.iter("text"):
        nodes.append(
            {
                "text": canonical("".join(node.itertext())),
...
def geometry_audit(path: pathlib.Path, oracle: dict[str, object]) -> dict[str, object]:
    root = element_tree.parse(path).getroot()
...
def main() -> int:
    ...
    versions = {
        "typst": command_version(["typst", "--version"]),
        "tika": command_version(tika_command("--version")),
        "poppler": command_version(["pdfinfo", "-v"]),
        "qpdf": command_version(["qpdf", "--version"]),
        "uv": command_version(["uv", "--version"]),
        "python": sys.version.splitlines()[0],
        "platform": platform.platform(),
        "locale": (run(["locale"], REPO_ROOT).stdout).strip(),
    }
    ...
    if any(returncode != 0 for returncode in command_results.values()):
        ...
        raise SystemExit(f"evidence command failed: {', '.join(failures)}; see {output}")
    ...
    ) = split_tika_visible_body(tika_text, expected_uris)
    raw_views = {
        ...
        "poppler_xml_text_raw": decoded_xml_text(output / "poppler.xml"),
    }
    ...
    for name, text in raw_views.items():
        ...
        text_views[name] = evaluate_text_view(text, oracle)
        text_views[canonical_name] = evaluate_text_view(canonical_text, oracle)

    work_authorization_views = {
        name: work_authorization_result(text, oracle["work_authorization"])
    ...
    fonts = parse_fonts(output / "pdffonts.txt")
    fonts_pass = bool(fonts) and all(
        font["embedded"] == font["subset"] == font["unicode"] == "yes"
        for font in fonts
    )
    image_count = count_images(output / "pdfimages.txt")
    qdf = qdf_audit(output / "qdf.pdf", oracle)
    geometry = geometry_audit(output / "poppler.xml", oracle)
    qpdf_clean = (
    ...
    gates = {
        ...
        "tika_full_raw_codepoints": not forbidden_codepoints(tika_text),
    ...
    report = {
        ...
        "tika": {
            ...
            "full_raw_forbidden_codepoints": forbidden_codepoints(tika_text),
        },
        "pdf": {
    ...
    print(f"Golden Resume automated local status: {automated_status}")
